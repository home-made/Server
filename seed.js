...
var seedData = () => {
  ...
  User.remove({}, function(err, deletedUsers) {
    if (err) {
      console.log(
        "There was a problem deleting the old users from the database. ",
        err
      );
    }
    ...
    User.create(UserData, function(error, newDB) {
      ...
      console.log(
        "The user data was successfully seeded to the Database. ",
        newDB
      );
      ...
      Dish.remove({}, function(err, deletedDishes) {
        if (err) {
          console.log(
            "There was a problem deleting the old dishes from the database. ",
            err
          );
        }
        ...
        Dish.create(DishData, function(error, newDB) {
          if (error) {
            console.log(
              "Dish seed data couldn't be saved to the Database. ",
              error
            );
          }
          console.log(
            "The dish data was successfully seeded to the Database. ",
            newDB
          );
